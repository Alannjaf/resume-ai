...
export const parseDeveloperHtmlToPdf = (htmlContent: string, styles: any): ParsedContent => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (!block.match(/^<[^>]*>$/)) {
      if (listType) {
        // We're inside a list - use // for both bullet and numbered lists
        ...
        currentList.push(
          <View key={`list-item-${currentKey++}`} style={{ 
            ...
            <Text style={[styles.text, { minWidth: 20, color: '#22c55e' }]}>{prefix}</Text>
            <View style={{ flex: 1 }}>
              {listItemContent}
            </View>
          </View>
...
const parseInlineElements = (content: string, styles: any): ReactElement[] => {
  ...
  for (const part of parts) {
    ...
    if (!part.match(/^<[^>]*>$/)) {
      const textStyle = [styles.text || { fontSize: 10, color: '#000000' }]
